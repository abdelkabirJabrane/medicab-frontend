pipeline {
    ...
    stages {
        stage('Build') {
            steps {
                // If you don't have NodeJS configured in Jenkins tools, 
                // you can rely on the Dockerfile to do the build.
                // Here we just prepare anything if needed, but since Dockerfile does multi-stage, 
                // we can just proceed to Docker build directly.
